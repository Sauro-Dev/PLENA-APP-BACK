...
public interface SessionRepository extends JpaRepository<Session, Long> {
    ...
    @Query("SELECT s FROM Session s WHERE s.therapist.idUser = :therapistId AND s.sessionDate BETWEEN :startDate AND :endDate ORDER BY s.sessionDate ASC")
    List<Session> findByTherapistIdAndSessionDateBetweenOrderBySessionDateAsc(
            @Param("therapistId") Long therapistId,
            @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);

    @Query("SELECT s FROM Session s WHERE s.patient.idPatient = :patientId AND s.sessionDate BETWEEN :startDate AND :endDate ORDER BY s.sessionDate ASC")
    List<Session> findByPatientIdAndSessionDateBetweenOrderBySessionDateAsc(
